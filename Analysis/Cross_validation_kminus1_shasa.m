...
function [trial_likelihood] = Cross_validation_kminus1_shasa(subject_id, continuum, block, width)
...
% Get the percent correct at each step in the training set
    for this_step = 1:7
        step = T_tmp(ismember(T_tmp.stimulus, ['Sa_Sha_Step_' ,num2str(this_step), '.wav']),:);
    ...
    if strcmp(obsv.selection{1}, 'Sha') %If they said 'da'
